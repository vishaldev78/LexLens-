...
for (const s of SAMPLES) {
  ...
  const firstLine = a.localized.en.summary.slice(0, 70);
  ...
  console.log(`  en.summary="${firstLine}…"`);

  check(`${s.id}: localized EN/HI/ES summaries present`, !!(a.localized.en.summary && a.localized.hi.summary && a.localized.es.summary));
  check(`${s.id}: next_steps in all 3 languages`, a.localized.en.next_steps.length > 0 && a.localized.hi.next_steps.length > 0 && a.localized.es.next_steps.length > 0);
...
check("generic: localized blocks present", !!(g.localized.en.summary && g.localized.hi.summary && g.localized.es.summary));

// red-severity safety: cheque bounce must include lawyer step in all languages
const cb = offlineAnalyze(SAMPLES[1].text);
check("cheque bounce (red): lawyer step in EN", cb.localized.en.next_steps.some((s) => /lawyer/i.test(s)));
check("cheque bounce (red): lawyer step in HI", cb.localized.hi.next_steps.some((s) => /वकील|अधिवक्ता/.test(s)));
check("cheque bounce (red): lawyer step in ES", cb.localized.es.next_steps.some((s) => /abogado/i.test(s)));
