...
import type { CaseBase, ExtractedClaim, ExtractedFact, Locale, LocalizedTexts, NoticeType, StatedDeadline } from "@/lib/lexlens/types";
...
export async function runAnalysis(noticeText: string): Promise<RunAnalysisResult> {
  const t0 = Date.now();
