...
export async function POST(req: NextRequest, { params }: Params) {
  try {
    ...
    const existing = await db.lawyerBrief.findFirst({ where: { noticeId: notice.id, sessionId: session.id }, select: { id: true } });
    if (existing) {
      await db.lawyerBrief.update({ where: { id: existing.id }, data: { content: payload, updatedAt: new Date() } });
    } else {
      await db.lawyerBrief.create({ data: { sessionId: session.id, noticeId: notice.id, content: payload, expiresAt: sessionExpiresAt() } });
    }
